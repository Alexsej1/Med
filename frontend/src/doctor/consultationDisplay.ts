...
export function labelForSymptomKey(key: string, labels?: Record<string, string> | null): string {
  if (!key) return "";
...
export function parseDiagnosisSnapshot(raw: Record<string, unknown> | null | undefined): DiagnoseResponse | null {
  ...
  return {
    ...
    needs_clarification: Boolean((raw as Partial<DiagnoseResponse>).needs_clarification),
    clarifying_questions: Array.isArray((raw as Partial<DiagnoseResponse>).clarifying_questions)
      ? ((raw as DiagnoseResponse).clarifying_questions ?? [])
      : [],
    max_probability: typeof maxP === "number" ? maxP : Number(preds[0]?.probability ?? 0),
  };
}
...
export function topDiagnosisLine(d: Record<string, unknown> | null | undefined): string {
  if (!d) return "—";
  const preds = (d as Partial<DiagnoseResponse>).predictions;
  if (!Array.isArray(preds) || preds.length === 0) return "—";
  const first = preds[0] as { disease?: string; probability?: number };
  if (!first?.disease) return "—";
  const p = first.probability;
  const pct = typeof p === "number" ? ` (${Math.round(p * 100)}%)` : "";
  return `${first.disease}${pct}`;
}

export function symptomsShort(keys: string[] | null, max = 5, labels?: Record<string, string> | null): string {
